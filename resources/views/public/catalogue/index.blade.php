...
@section('title', 'Catalogue véhicules – MBPRESTIGE')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    {{-- En-tête --}}
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Catalogue véhicules</h1>
        <p class="text-gray-500 text-sm mt-1">{{ $listings->total() }} véhicule(s) disponible(s)</p>
    </div>
        ...
        <div class="lg:hidden fixed inset-0 z-50 bg-black/50" x-show="filtersOpen" x-cloak @click="filtersOpen=false">
            <div class="absolute left-0 top-0 h-full w-80 bg-white overflow-y-auto p-4" @click.stop>
                <div class="flex justify-between items-center mb-4">
                    ...
                    <button @click="filtersOpen=false" class="text-gray-400 hover:text-gray-600">✕</button>
                </div>
                @include('components.filter-sidebar', ['filters' => $filters])
            </div>
        </div>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center justify-between mb-5 gap-4 flex-wrap">
                ...
                <div class="flex flex-wrap gap-2 flex-1">
                    @foreach(request()->except(['page','sort']) as $key => $val)
                        ...
                        <span class="inline-flex items-center gap-1 bg-blue-50 text-blue-700 text-xs font-medium px-2 py-1 rounded-full">
                            {{ $val }}
                            <a href="{{ request()->fullUrlWithQuery([$key => null]) }}" class="hover:text-blue-900">✕</a>
                        </span>
                        @endif
                    @endforeach
                </div>
                ...
                <form method="GET" id="sort-form">
                    ...
                    <select name="sort" onchange="document.getElementById('sort-form').submit()"
                        ...
                        <option value="published_at" @selected(request('sort','published_at')==='published_at')>Plus récents</option>
                        ...
                        <option value="ends_at"      @selected(request('sort')==='ends_at')>Fin enchère</option>
                    </select>
                </form>
            </div>
            ...
            @else
                <div class="text-center py-20 text-gray-400">
                    ...
                    <p class="text-lg font-medium text-gray-500">Aucun véhicule trouvé</p>
